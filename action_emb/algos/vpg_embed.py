...
def VPG_embed(cnf):
    ...
    buffer = VPGFlexBuffer(obs_dim, act_dim, cnf_train['buffer_size'], cnf_train['gamma'], cnf_train['lam'])
    ...
    try:
        ...
        if cnf['embed']['embed_module'] == 'SASEmbeddingModule' or cnf['embed'][
            'embed_module'] == 'AutoEncoderEmbeddingModule':
            pretrain_SAS_buffer(cnf, obs_dim, act_dim, cnf_train, ac, episode_num, env, logger)
        else:
        ...
        for epoch in range(cnf_train['num_epochs']):
            terminal = False
            episode_num, total_env_interacts = run_epoch(episode_num, total_env_interacts, ep_ret_all,
                                                         cnf_train['steps_per_epoch'],
                                                         env, ac, logger, buffer, cnf_train)
